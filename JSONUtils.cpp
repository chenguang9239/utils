...
std::string JSONUtils::getStringField(const std::string &json,
...
std::vector<std::string> JSONUtils::getStrArrayField(const std::string &json,
                                                     const std::string &field) {
...
std::string JSONUtils::removeField(const std::string &json,
                                   const std::string &field) {
  std::string res;
  try {
    rapidjson::Document d;
...
JSONUtils::getLeftJoinJSON(const std::string &json1, const std::string &json2,
  ...
  try {
    ...
    d3.SetObject();
    d4.SetObject();
    auto countPair = getLeftJoinDocument(d1, d2, d3, d4, onlyGetDifferent,
                                         doubleDeviation, exclusion);
    if (countPair.first > 0) {
      res = std::pair<std::string, std::string>(documentToJSON(d3),
                                                documentToJSON(d4));
    }
  } catch (const std::exception &e) {
...
std::pair<int, int>
JSONUtils::getLeftJoinDocument(rapidjson::Document &d1, rapidjson::Document &d2,
    ...
    if (d1.IsObject() && d1.MemberCount() > 0) {
      if (d2.IsObject() && d2.MemberCount() > 0) {
        ...
        for (auto &&it = d1.MemberBegin(); it != d1.MemberEnd(); ++it) {
          if (emptyExclusion || exclusion.count(it->name.GetString()) == 0) {
            if (d2.HasMember(it->name)) {
              isDifferent =
                  checkDifferent(it->value, d2[it->name], doubleDeviation);
              if (!onlyGetDifferent || isDifferent) {
                ...
                d3.AddMember(
                    rapidjson::Value(it->name, d3.GetAllocator()).Move(),
                    rapidjson::Value(it->value, d3.GetAllocator()).Move(),
                    d3.GetAllocator());
                d4.AddMember(
                    rapidjson::Value(it->name, d4.GetAllocator()).Move(),
        ...
        for (const auto &field : exclusion) {
          if (d3.HasMember(field.c_str())) {
            d3.RemoveMember(field.c_str());
          }
        }
...
bool JSONUtils::isValidJSON(const std::string &json) {
  ...
  try {
    ...
    res = d.IsObject(); // if json string is "null", there will no parse error,
                        // but d is not an object
  } catch (const std::exception &e) {
...
bool JSONUtils::isEqualJSON(
    const std::string &json1, const std::string &json2,
  ...
  try {
    ...
    d3.AddMember(rapidjson::Value("fieldNumber").Move(),
                 rapidjson::Value(d1.IsObject() ? d1.MemberCount() : 0).Move(),
                 d3.GetAllocator());
    d4.AddMember(rapidjson::Value("fieldNumber").Move(),
                 rapidjson::Value(d2.IsObject() ? d2.MemberCount() : 0).Move(),
                 d4.GetAllocator());
    ...
    if (d1 == d2) {
      ...
      auto countPair =
          getLeftJoinDocument(d1, d2, d3, d4, true, doubleDeviation, exclusion);
...
std::string
JSONUtils::getLeftUniqueJSON(const std::string &json1, const std::string &json2,
                             const std::set<std::string> &exclusion) {
  std::string res;
  try {
...
int JSONUtils::getLeftUniqueDocument(rapidjson::Document &d1,
  ...
  try {
    ...
    if (d1.IsObject() && d1.MemberCount() > 0) {
      if (d2.IsObject() && d2.MemberCount() > 0) {
        ...
        for (auto &&it = d1.MemberBegin(); it != d1.MemberEnd(); ++it) {
          if ((emptyExclusion || exclusion.count(it->name.GetString()) == 0) &&
              !d2.HasMember(it->name)) {
            d3.AddMember(
                rapidjson::Value(std::move(it->name), d3.GetAllocator()).Move(),
                rapidjson::Value(std::move(it->value), d3.GetAllocator())
                    .Move(),
                d3.GetAllocator());
            ++leftUniqueCount;
          }
...
std::string JSONUtils::documentToJSON(rapidjson::Document &d,
                                      bool permitNullOrEmpty) {
  std::string res;
  try {
...
bool JSONUtils::checkDifferent(const rapidjson::Value &v1,
  ...
  if (v1.IsDouble() && v2.IsDouble()) {
    ...
    if (!isDifferent) {
      for (auto it1 = obj1.MemberBegin(); it1 != obj1.MemberEnd(); ++it1) {
        isDifferent =
            !obj2.HasMember(it1->name) ||
            checkDifferent(it1->value, obj2.FindMember(it1->name)->value,
                           doubleDeviation);
        if (isDifferent)
          break;
      }
...
bool JSONUtils::JSONToDocument(const std::string &json,
                               rapidjson::Document &d) {
  bool res = false;
  try {
    d.Parse(json.c_str());
    ...
    LOG_ERROR << "JSONToDocument exception: " << e.what();
  }
  return res;
}

bool JSONUtils::AddMongoDateStr(rapidjson::Document &d) {
  bool res = false;
  try {
    if (!d.IsObject()) {
    ...
    for (auto &&it = d.MemberBegin(); it != d.MemberEnd(); ++it) {
      if (it->value.IsObject()) {
        if (it->value.HasMember("$date")) {
          auto &&obj = it->value.GetObject();
          auto &time_stamp = obj.FindMember("$date")->value;
          if (!time_stamp.IsInt64()) {
            continue;
          }
          it->value.AddMember(
              rapidjson::Value("date_str", d.GetAllocator()).Move(),
              rapidjson::Value(
                  dateUtil::timeStampToLocalTime(time_stamp.GetInt64() / 1000)
                      .c_str(),
                  d.GetAllocator())
                  .Move(),
              d.GetAllocator());
    ...
    LOG_ERROR << "AddDateStr exception: " << e.what();
  }
  return res;
}